...
export default function Hero({ onOpenBooking }: HeroProps) {
  ...
  return (
    <section className="relative overflow-hidden pt-8 sm:pt-16 lg:pt-24 xl:pt-28 pb-14 sm:pb-20 lg:pb-24 bg-gradient-to-b from-[#fbf8f2] via-white to-[#f8fbfd]">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-10 items-center">
          ...
          {/* Left Column: Direct Canvas Layout with Above-The-Fold Mobile Hook */}
          <div className="lg:col-span-7 relative z-10 space-y-4 sm:space-y-6">
            
            {/* Real Clinic Location & Availability Badge */}
            <div className="inline-flex items-center gap-2 px-3 py-1 sm:px-3.5 sm:py-1.5 rounded-full bg-amber-50/80 border border-amber-200/60 text-[11px] sm:text-xs font-semibold text-[#8c6210]">
              <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
              <span>Mayor Jaldon St., Canelar • Zamboanga City</span>
            </div>

            {/* Editorial Title with Refined Serif Accent - 1 Row Layout on Tablet/Desktop */}
            <div className="space-y-1.5 sm:space-y-2 min-h-0 sm:min-h-[84px] lg:min-h-[92px] flex flex-col justify-start">
            ...
            {/* Short friendly clinic introduction */}
            <p className="text-slate-600 text-sm sm:text-base lg:text-lg leading-relaxed max-w-[54ch] min-h-0 sm:min-h-[84px] lg:min-h-[80px]">
              {t.hero.description}
            </p>

            {/* Action Buttons & Verified Accreditation - High-Contrast Primary CTA */}
            <div className="flex flex-wrap items-center gap-3 pt-1">
              <button
                type="button"
                onClick={onOpenBooking}
                className="w-full sm:w-[220px] inline-flex items-center justify-center gap-2.5 bg-gradient-to-r from-[#dfba6b] via-[#c5a059] to-[#a07823] hover:from-[#e8c679] hover:via-[#d4af37] hover:to-[#b3882a] text-slate-950 font-extrabold text-sm sm:text-base h-13 sm:h-12 rounded-2xl shadow-xl shadow-amber-900/20 hover:shadow-2xl hover:scale-[1.01] active:scale-[0.98] transition-all cursor-pointer border border-amber-200/70 whitespace-nowrap shrink-0"
              >
